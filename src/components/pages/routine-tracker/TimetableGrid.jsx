...
// https://api.aladhan.com/v1/timingsByCity/18-06-2025?city=Cairo&country=Egypt&method=5

- statics for finish tasks every day
- when the time get show popup ( have music work and have more option stop or snoze fter what mins and missed)
- in case no any routine how show the page 


 - when repeat any thing i dont' repeat it on the array find whay to repeat it in the table but with add it again and agin in the array
    because the name is the same i dont' will chagne it that will chnge only the time and his status



    ❌ focus day  it doesn't work
    ❌ edit doesn't work

    if the card checked handle the colors of actions
*/
...
import { Edit, Focus, Trash } from 'lucide-react';
...
const TimetableGrid = ({ routines, setRoutines, isLoading }) => {
    ...
    // Your existing effect, modified to check for user interaction

    useEffect(() => {
        const dueRoutines = routines.filter(
            routine => routine.status === 'pending' && checkIfNow(routine.timeStart) && (!alertingRoutine || alertingRoutine.id !== routine.id), // Don't re-alert the same routine
        );
    ...
    const checkIfNow = isoString => {
        const now = new Date(); // احصل على الوقت الحالي الآن
    ...
    const handleSnooze = (id, mins) => {
        ...
        setTimeout(() => {
            ...
            toast.info(`Reminder for ${routine?.title}`);
        }, mins * 60 * 1000);
    };
    ...
    useEffect(() => {
        ...
        const items = [];
        items.push(<div key='head-time' className='w-[full] h-[45px] block bg-background-subtle border border-border'></div>);
        ...
        visibleDays.forEach((day, i) => {
            ...
            items.push(
                <div
                    ...
                    className={clsx('border hover:bg-primary hover:text-white hover:scale-[1.02] border-border text-sm !px-[50px] p-1 text-center font-semibold relative transition-all duration-300 cursor-pointer', day.isToday ? 'bg-primary text-white ring-2 ring-primary' : isFocusedDay ? 'bg-primary text-white ring-2 ring-primary' : 'bg-background-subtle')}
                    style={{
            ...
            visibleDays.forEach((day, colIdx) => {
                ...
                items.push(
                    <motion.div
                        ...
                        animate='animate'
                        exit='exit'
                        className={clsx('flex flex-col gap-2 p-2 transition-all outline-3 outline-border/5 outline', isFocusedDay ? 'bg-blue-50' : '')}
                        style={{
    ...
    return (
        <div className=''>
            ...
            <div className='border border-border/70 shadow-sm relative rounded-2xl text-center w-full bg-white/80 backdrop-blur-[2px] p-0 min-h-[50vh]'>
                <audio id='alert' controls ref={alertSound} className='hidden' src='/sounds/religion/alert8.mp3' preload='auto' />
            ...
            <Dialog title='Confirm Delete' open={isDeleting !== null} onClose={() => setIsDeleting(null)}>
                <div className='p-6 space-y-5'>
                    ...
                    <div className='flex justify-end gap-2 pt-4'>
                        <Button onClick={() => setIsDeleting(null)} label='Cancel' color='primary' />
                        <Button onClick={handleDelete} label='Delete' color='red' />
                    </div>
                </div>
            </Dialog>
            ...
            {alertingRoutine && (
                <Dialog
                    key={alertingRoutine.id}
                    title='⏰ Reminder! What do you want to do?'
                    open={true}
                    onClose={() => {
                    ...
                    <div className='p-6 space-y-5'>
                        <div className='text-sm text-gray-600'>
                            <p>
                                <strong>Routine:</strong> {alertingRoutine.title}
                            </p>
                            <p>
                                <strong>Scheduled Time:</strong> {displayTime(alertingRoutine.timeStart)}
                            </p>
                        </div>
                        ...
                        <div className='flex justify-end gap-2 pt-4'>
                            ...
                            <select
                                ...
                                onChange={e => {
                                    ...
                                    if (!isNaN(value)) {
                                        handleSnooze(alertingRoutine.id, value);
                                        setAlertingRoutine(null);
                                    }
                                }}
            ...
            {/* {routines.map(routine => {
                const shouldShowDialog = routine.status === 'pending' && checkIfNow(routine.timeStart);
                if (!shouldShowDialog) return null;
                if(shouldShowDialog) setShowAlert(true);

                return (
                    <Dialog key={routine.id} title='⏰ Reminder! What do you want to do?' open={true} onClose={() => handleMissed(routine.id)}>
                        <div className='p-6 space-y-5'>
                            <div className='text-sm text-gray-600'>
                                <p>
                            ...
                            <div className='flex justify-end gap-2 pt-4'>
                                ...
                                <select
                                    ...
                                    onChange={e => {
                                        const value = parseInt(e.target.value);
                                        if (!isNaN(value)) handleSnooze(routine.id, value);
                                    }}
                                    className='text-xs bg-gray-100 border rounded px-2 py-1'>
                                    <option disabled value=''>
                                        Snooze...
