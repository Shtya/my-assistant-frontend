...
export default function SelectDefault({cnLabel ="", label = '', options = [], value, onChange, className = '' }) {
    ...
    return (
        <div className={`relative w-full space-y-1 ${className}`} ref={selectRef}>
            {label && <label className={` ${cnLabel}  block text-sm font-medium text-gray-700 mb-2 `}>{label}</label>}
