/* 
  ...
  - and use lucide-react to import any icons 


  - this s my componnents use it 
...
import { Code, Star, Eye, Plus, X, Save, Upload, Download, ChevronDown, ChevronUp, Check, Filter, Edit, Edit2 } from 'lucide-react';
import Input2 from '@/components/atoms/Input2';
import Button from '@/components/atoms/Button';
import CheckBox from '@/components/atoms/CheckBox';
import SelectDefault from '@/components/atoms/SelectDefault';
...
const KnowledgeBase = () => {
    ...
    // Initialize questions and readCards from localStorage
    useEffect(() => {
        const savedQuestions = localStorage.getItem('knowledgeBaseQuestions');
        const savedReadCards = localStorage.getItem('knowledgeBaseReadCards');

        // setQuestions(savedQuestions ? JSON.parse(savedQuestions) : qs_progrmming);
        setQuestions(qs_progrmming);
        setReadCards(savedReadCards ? JSON.parse(savedReadCards) : []);
    }, []);

    // Save questions and readCards to localStorage whenever they change
    useEffect(() => {
        localStorage.setItem('knowledgeBaseQuestions', JSON.stringify(questions));
