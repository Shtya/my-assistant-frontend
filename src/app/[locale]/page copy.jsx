...
import { Eye, EyeClosedIcon, EyeIcon, PlusIcon, Sparkles, SparklesIcon, FilterIcon, SettingsIcon, Search, X, LayoutTemplate, Loader2, EyeClosed } from 'lucide-react';
import { useState, useEffect, useRef } from 'react';
import ReactDOM from 'react-dom';
import Select from '@/components/atoms/Select';

// Dialog Component
function Dialog({ cn, open, width, height, onClose, title, children }) {
...
// Button Component
function Button({ label, Icon, onClick, color = 'primary', cn = '', infoTooltip, title }) {
  const colors = {
  ...
  return (
    <button
      title={title}
      onClick={onClick}
      className={`flex items-center justify-center gap-2 rounded-lg p-2 transition ${colors[color]} ${cn}`}>
      {Icon && Icon}
...
export default function KanbanBoard() {
  ...
  const filteredCards = cards.filter(card => {
    ...
    const matchesMembers = filter.members.length === 0 || 
                          cardMembers.some(cm => cm.card_id === card.id && filter.members.includes(cm.user_id));
    const matchesLabels = filter.labels.length === 0 || 
                         cardLabels.some(cl => cl.card_id === card.id && filter.labels.includes(cl.label_id));
  ...
  const userBoards = boards.filter(board => 
    ...
    board.created_by === currentUser.id || 
    boardMembers.some(bm => bm.board_id === board.id && bm.user_id === currentUser.id)
  ...
  const inviteToBoard = (boardId, email) => {
    ...
    if (boardMembers.some(bm => bm.board_id === boardId && bm.user_id === user.id)) {
      return alert('User is already a member');
    }
  ...
  const handleAiQuery = () => {
    ...
    setTimeout(() => {
      ...
      if (query.includes('risk') || query.includes('problem')) {
        ...
        response = `Found ${delayed} delayed tasks out of ${predictions.length}:\n` +
                   predictions
  ...
  const analyzeRisks = () => {
    const risks = cards
      ...
      .map(card => ({
        cardId: card.id,
        title: card.title,
        riskType: card.due_date && new Date(card.due_date) < new Date() ? 'Overdue' : 
                 card.status === 'blocked' ? 'Blocked' : 
                 card.time_spent / card.estimate_time > 1.5 ? 'Time Overrun' : 'Due Soon',
  ...
  const suggestions = [
    'Show me potential risks in my project',
    'Give me a time tracking report',
    'What tasks are likely to be delayed?',
    'List all my boards'
  ];
    ...
    <div className='min-h-screen rtl:mr-[10px] ltr:ml-[10px] w-[calc(100%-10px)] relative'>
      ...
      <div className='relative z-[10] !flex mt-4 !flex-row !items-center !justify-between !p-4 card w-full text-text-base'>
        <div className='flex items-center space-x-2'>
          ...
          {currentBoard && (
            <span className={`px-2 h-5 rtl:!ml-[40px] ltr:!mr-[40px] text-[10px] capitalize flex items-center justify-center rounded ${
              currentBoard.visibility === 'public' ? 'bg-primary/90 text-white' : 
              currentBoard.visibility === 'team' ? 'bg-green-100 text-green-800' : 
            ...
            {focusMode && (
              <div className="animate-fade-in">
                <Select
                  ...
                  value={selectedUser} 
                  onChange={val => setFocusedUserId(val?.key || null)} 
                />
              </div>
            )}
            <Button 
              title="Focus on your cards" 
              color="white" 
              Icon={focusMode ? <EyeClosed className="h-4 w-4 text-red-500" /> : <Eye className="h-4 w-4 text-blue-500" />} 
              onClick={() => setFocusMode(!focusMode)} 
            />
          </div>

          <Button 
            title='Filters the cards' 
            color='white' 
            Icon={<FilterIcon className='h-4 w-4 text-gray-500 group-hover:text-blue-500' />} 
          ...
          <Button 
            ...
            color='white' 
            cn='' 
            Icon={<Sparkles className='text-purple-500 h-4 w-4' />} 
        ...
        <div className='flex items-center justify-end space-x-2 flex-1'>
          <div className='relative flex items-center gap-2 px-2 h-10 py-2 border rounded-lg w-fit'>
            ...
            <input 
              ...
              className='outline-none flex-1 w-[150px] focus:!w-[200px] duration-500' 
              value={searchQuery} 
          ...
          <Button 
            color='white' 
            Icon={<SettingsIcon className='text-primary w-4 h-4' />} 
            onClick={() => setActiveModal('board-settings')} 
          />

          <div className='relative'>
            <button 
              className='p-2 rounded-md duration-300 hover:bg-gray-100 relative' 
              ...
              {notificationsData.filter(n => !n.read && n.user_id === currentUser.id).length > 0 && (
                ...
                <Reorder.Item
                  ...
                  <div
                    ...
                    <div className='flex w-full justify-between items-center mb-3'>
                      ...
                      <span className='inline-flex items-center justify-center w-5 h-5 text-[10px] font-medium rounded-full bg-blue-100 text-blue-700'>
                        {listCards.length}
                      </span>
                    </div>
                    ...
                    <div className='space-y-3 w-full'>
                      {listCards.map((card, index) => (
                        <div
                          key={card.id}
                          className={`border border-border/50 bg-background-subtle duration-300 shadow-inner rounded-lg p-3 hover:bg-background-muted hover:scale-[1.01] cursor-pointer ${
                            focusMode ? 'border-1 bg-primary/10 !border-primary/40' : ''
                          }`}
                          draggable
                          style={{ transition: '.5s' }}
                          onDragStart={e => {
                            ...
                            <div className='flex items-center space-x-2'>
                              {card.due_date && (
                                <div className={`text-[10px] flex items-center justify-center ${
                                  new Date(card.due_date) < new Date() && card.status !== 'done' ? 
                              ...
                              {card.issue_type && (
                                <span className={`inline-block text-[10px] font-medium px-2 py-[2px] rounded-md capitalize shadow-inner ${
                                  {
                            ...
                            <div className='flex items-center'>
                              {getCardMembers(card.id).length > 0 && (
                                <div className='flex -space-x-1'>
                                  ...
                                  {getCardMembers(card.id).length > 3 && (
                                    <span className='h-5 w-5 rounded-full bg-primary border-2 border-white text-[10px] text-white flex items-center justify-center'>
                                      +{getCardMembers(card.id).length - 3}
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className='h-[5px] w-full bg-neutral-200 rounded-full overflow-hidden shadow-sm'>
                                <div
                                  className={`h-full rounded-full transition-all duration-300 ease-in-out ${
                                    card.time_spent / card.estimate_time > 1 ? 'bg-red-500' : 'bg-blue-500'
                                  }`}
                                  style={{
              ...
              {userBoards.map(board => (
                <div
                  ...
                  className={`p-3 border rounded-lg cursor-pointer hover:bg-gray-50 ${
                    currentBoardId === board.id ? 'bg-blue-50 border-blue-200' : ''
                  }`}
                  onClick={() => {
                  ...
                  <div className='flex justify-between items-center'>
                    ...
                    <span className={`text-xs px-2 py-1 rounded ${
                      board.visibility === 'public' ? 'bg-blue-100 text-blue-800' : 
                      board.visibility === 'team' ? 'bg-green-100 text-green-800' : 
                      'bg-gray-100 text-gray-800'
                    }`}>
                      {board.visibility}
                    </span>
      ...
      {/* Create Board Modal */}
      <Dialog 
        title={<h2 className='text-xl font-bold'>Create New Board</h2>} 
        width={'400px'} 
        open={activeModal === 'create-board'} 
      ...
      <Dialog 
        title={<h2 className='text-xl font-bold'>Your Boards</h2>} 
        width={'400px'} 
        open={activeModal === 'board-selector'} 
        onClose={() => setActiveModal(null)} 
        cn='bg-white'>
        <div className='space-y-2 mb-4'>
          {userBoards.map(board => (
      ...
      {/* Notifications Modal */}
      <Dialog 
        title={<h2 className='text-xl font-bold'>Notifications</h2>} 
        height='230px' 
        width={'400px'} 
        ...
        {notificationsData
          ...
          .map(notification => (
            <div
              key={notification.id}
              className={`p-3 border-b cursor-pointer hover:bg-blue-100 duration-300 ${
        ...
        {notificationsData.filter(n => n.user_id === currentUser.id).length === 0 && (
      ...
      <Dialog 
        title={<h2 className='text-xl font-bold'>Board Settings</h2>} 
        height='70%' 
        width={'400px'} 
        open={activeModal === 'board-settings'} 
        onClose={() => setActiveModal(null)} 
        cn='bg-white'>
              ...
              <label key={type} className='flex items-center space-x-2 mb-2'>
                ...
                <div>
                  ...
                  <p className='text-xs text-gray-500'>
                    {type === 'public' ? 'Anyone can see this board' : 
                     type === 'team' ? 'Only team members can see this board' : 
          ...
          <div className='flex'>
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-r-lg text-sm font-medium transition ${
                isAiProcessing || !aiQuery ? 'bg-gray-300 text-gray-600 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'
              }`}>
      ...
      {/* Filters Modal */}
      <Dialog 
        title={<h2 className='text-2xl font-bold flex items-center gap-2'>Filter Tasks</h2>} 
        height='70%' 
        width={'400px'} 
                ...
                {users.map(user => (
                  <label key={user.id} className='flex items-center gap-2'>
                    <input 
                      ...
                      onChange={() => handleCheckbox('members', user.id)} 
            ...
            <div>
              ...
              <div className='flex flex-wrap gap-2'>
                {boardLabels.map(label => (
                  <button 
                    key={label.id} 
                    onClick={() => handleCheckbox('labels', label.id)} 
                    className={`text-xs font-medium px-3 py-1 rounded-md transition ring-offset-2 focus:outline-none ${
                      filter.labels.includes(label.id) ? 'ring-2 ring-blue-400' : 'hover:opacity-90'
                    }`} 
                    style={{ backgroundColor: label.color }}>
                    {label.title}
                  </button>
              ...
              <div className='flex flex-wrap gap-2'>
                {issueTypes.map(type => (
                  <button 
                    ...
                    className={`px-3 py-1 text-sm rounded-lg shadow-inner transition ${
                      filter.types.includes(type) ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                    }`}>
            ...
            <div>
              ...
              <div className='flex flex-wrap gap-2'>
                {statusTypes.map(status => (
                  <button 
                    key={status} 
                    onClick={() => handleCheckbox('statuses', status)} 
                    className={`px-3 py-1 text-sm rounded-lg shadow-inner transition ${
                      filter.statuses.includes(status) ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                    }`}>
                    {status
                      .split('-')
            ...
            <div className='flex justify-between items-start mb-6 border-b pb-4'>
              <div className='space-y-1'>
                ...
                <p className='text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1'>
                  In list
                  <span className='ml-1 inline-block rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 px-3 py-0.5 text-xs font-semibold'>
                    {lists.find(l => l.id === activeCard.list_id)?.title || 'Unknown'}
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className='flex flex-col md:flex-row gap-6'>
              <div className='flex-1'>
                      ...
                      {activeCard.custom_fields.map(field => (
                        <div key={field.id} className='space-y-1'>
                          ...
                          {field.type === 'select' ? (
                            <select 
                              value={field.value} 
                              onChange={e => updateCustomField(activeCard.id, field.id, e.target.value)} 
                ...
                <div className='mb-6'>
                          ...
                          {items.map(item => (
                            <div key={item.id} className='flex items-center'>
                              <input 
                                ...
                                onChange={() => toggleChecklistItem(item.id, checklist.id)} 
                                className='mr-2' 
                              />
                  ...
                  {getCardChecklists(activeCard.id).length === 0 && (
                  ...
                  <div className='space-y-4'>
                    ...
                    {getCardComments(activeCard.id).map(comment => (
                      <div key={comment.id} className='flex'>
                        <img 
                          src={getUserById(comment.user_id)?.avatar} 
                          alt={getUserById(comment.user_id)?.name} 
                        ...
                        <div className='flex-1'>
                          <div className='bg-gray-50 p-3 rounded-lg'>
                            <div className='flex justify-between'>
                              ...
                              <p className='text-xs text-gray-500'>
                                {new Date(comment.timestamp || new Date().toISOString()).toLocaleString()}
                              </p>
                            </div>
                    ...
                    {getCardComments(activeCard.id).length === 0 && (
                ...
                <div>
                  ...
                  <div className='space-y-3'>
                    {getCardActivities(activeCard.id)
                      ...
                      .map(activity => (
                        <div key={activity.id} className='flex'>
                          <img 
                            src={getUserById(activity.user_id)?.avatar} 
                            alt={getUserById(activity.user_id)?.name} 
                    ...
                    {getCardActivities(activeCard.id).length === 0 && (
                    ...
                    {getCardMembers(activeCard.id).length > 0 ? (
                      getCardMembers(activeCard.id).map(member => (
                        <div key={member.user_id} className='flex items-center'>
                          <img 
                            src={getUserById(member.user_id)?.avatar} 
                            alt={getUserById(member.user_id)?.name} 
                            className='h-8 w-8 rounded-full mr-2' 
                          />
                          <span>{getUserById(member.user_id)?.name}</span>
                          <button 
                            className='ml-auto text-red-500 hover:text-red-700' 
                            onClick={() => toggleCardMember(activeCard.id, member.user_id)}>
                  ...
                  <div className='space-y-2'>
                    {automationRules.map(rule => (
                      <div key={rule.id} className='p-2 bg-gray-50 rounded'>
                        ...
                        <p className='text-xs text-gray-500 mt-1'>
                          When: {rule.trigger.type === 'status_change' ? `status changes to ${rule.trigger.to}` : 
                                rule.trigger.type === 'move' ? `moved to list ${rule.trigger.to_list}` : 
                                rule.trigger.type === 'due_date' ? `${rule.trigger.days_before} days before due date` : ''}
                        </p>
                      </div>
                    ))}
                  </div>
